...
const int CHANNELS = 80;
const int PAD = 16;
const int STEPPING_PAD = 18;
...
class Caller {
  public:
    ...
    PW<BATCH_SIZE, 9, CHANNELS> pw_in;
    PW<BATCH_SIZE, CHANNELS/2, 5> pw_out;
    Block<BATCH_SIZE, CHANNELS> blocks[5];
    BlockC<BATCH_SIZE, CHANNELS> blockc;
    BlockC2<BATCH_SIZE, CHANNELS> blockc2;
    float *ib, *ob, *inp_im2col, *inp, *out, *buf2, *bufd2s;

    Caller() : weights(load_weight("net.txt")),
        pw_in(weights["e.encoder.0.conv.0.conv.weight"]),
        pw_out(weights["out.weight"]), 
        blocks {
            Block<BATCH_SIZE, CHANNELS>(weights, "e.encoder.1"),
            Block<BATCH_SIZE, CHANNELS>(weights, "e.encoder.2"),
            Block<BATCH_SIZE, CHANNELS>(weights, "e.encoder.3"),
            Block<BATCH_SIZE, CHANNELS>(weights, "e.encoder.4"),
            Block<BATCH_SIZE, CHANNELS>(weights, "e.encoder.5")
        },
        blockc(weights, "e.encoder.6"),
        blockc2(weights, "e.encoder.7")
    {
        ib = (float*) aligned_alloc(ALIGN, CHANNELS*sizeof(float));
        ...
        inp_im2col = (float*) aligned_alloc(ALIGN, (BATCH_SIZE)*9*sizeof(float));
        inp = (float*) aligned_alloc(ALIGN, (BATCH_SIZE+2*PAD)*80*sizeof(float));
        out = (float*) aligned_alloc(ALIGN, (BATCH_SIZE+2*PAD)*80*sizeof(float));
        buf2 = (float*) aligned_alloc(ALIGN, (BATCH_SIZE+2*PAD)*80*sizeof(float));
        bufd2s = (float*) aligned_alloc(ALIGN, (BATCH_SIZE/3+2*PAD)*160*sizeof(float));
        memset(inp_im2col, 0, (BATCH_SIZE)*9*sizeof(float));
        memset(inp, 0, (BATCH_SIZE+2*PAD)*80*sizeof(float));
        memset(out, 0, (BATCH_SIZE+2*PAD)*80*sizeof(float));
        memset(buf2, 0, (BATCH_SIZE+2*PAD)*80*sizeof(float));
    ...
    float* call_chunk(float* inpx) {
        ...
        blocks[0].calc(inp + PAD*80, out + PAD*80, bufd2s + PAD*160);
        blocks[1].calc(out + PAD*80, inp + PAD*80, bufd2s + PAD*160);
        blocks[2].calc(inp + PAD*80, out + PAD*80, bufd2s + PAD*160);
        blocks[3].calc(out + PAD*80, inp + PAD*80, bufd2s + PAD*160);
        blocks[4].calc(inp + PAD*80, out + PAD*80, bufd2s + PAD*160);
    ...
    py::array_t<float> call(py::array_t<float, py::array::c_style | py::array::forcecast> array) {
        ...
        auto result = py::array_t<float>((array.size()+2)/3*5);
        py::buffer_info result_buf = result.request();
        float* result_ptr = (float*) (result_buf.ptr);

        int out_pos = 0;
        for (int i = 0; i < array.size() - 6*STEPPING_PAD; i += BATCH_SIZE*3-6*STEPPING_PAD) {
            float *out;
            int pad_start, pad_end;
            if (i + BATCH_SIZE*3 < array.size()) {
//                printf("call %d\n", i);
                out = call_chunk(ptr+i);
                pad_start = i == 0 ? 0 : STEPPING_PAD;
                pad_end = STEPPING_PAD;
            } else {
                out = call_chunk(ptr+array.size()-BATCH_SIZE*3);
                pad_end = 0;
                pad_start = (i - (array.size() - BATCH_SIZE*3))/3 + STEPPING_PAD;
            }
